...
if (!$pp['InstallationPath']) { $pp['InstallationPath'] = 'C:\CudaText-beta' }

$packageArgs = @{
  packageName = $env:ChocolateyPackageName
  destination = $pp['InstallationPath']
  file64      = "$toolsDir\cudatext-win-x64-1.85.0.0.zip"
}
...
$shortcutFilePath = Join-Path $programs "CudaText Beta.lnk"
